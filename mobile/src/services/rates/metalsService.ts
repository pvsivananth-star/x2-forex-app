...
const METAL_UNITS: Record<string, {base: string; factor: number}> = {
    ...
    XAL_1LB: {base: 'XAL', factor: 1 / POUNDS_PER_METRIC_TONNE},
    XNI_1LB: {base: 'XNI', factor: 1 / POUNDS_PER_METRIC_TONNE},
...
async function fetchYahooSeries(ticker: string): Promise<{
    current: number;
    closes: number[];
} | null> {
    try {
...
export async function fetchMetalsRates(
    ...
    const data: MarketAsset[] = METAL_CATALOG
        .map(asset => {
            ...
            return {
                ...
                changePct: percentageChange(rate, referenceRate),
            };
        })
    ...
    return {
        data,
        isOffline: data.filter(asset => asset.symbol !== 'USD').length === 0,
        timestamp: Date.now(),
    };
}

...
const TENOR_TRADING_DAYS: Record<Tenor, number> = {'1D': 1, '1W': 5, '1M': 21, '3M': 63, '6M': 126, '1Y': 252};
const METAL_TICKERS: Record<string, string> = {XAU: 'GC=F', XAG: 'SI=F', XPT: 'PL=F', XPD: 'PA=F', XCU: 'HG=F', XAL: 'AL=F', XNI: 'NI=F', XZN: 'ZNC=F', XPB: 'LED=F'};
const POUNDS_PER_METRIC_TONNE = 2204.62262185;
const METAL_UNITS: Record<string, {base: string; factor: number}> = {
    XAU_1OZ: {base: 'XAU', factor: 1}, XAG_1OZ: {base: 'XAG', factor: 1}, XPT_1OZ: {base: 'XPT', factor: 1}, XPD_1OZ: {base: 'XPD', factor: 1}, XCU_1LB: {base: 'XCU', factor: 1}, XAL_1LB: {base: 'XAL', factor: 1 / POUNDS_PER_METRIC_TONNE}, XNI_1LB: {base: 'XNI', factor: 1 / POUNDS_PER_METRIC_TONNE}, XZN_1LB: {base: 'XZN', factor: 1 / POUNDS_PER_METRIC_TONNE}, XPB_1LB: {base: 'XPB', factor: 1 / POUNDS_PER_METRIC_TONNE},
};
...
async function fetchYahooSeries(ticker: string): Promise<{current: number; closes: number[]} | null> {
    try {
        const response = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(ticker)}?interval=1d&range=1y`);
        if (!response.ok) return null;
        const json = await response.json();
        const chart = json?.chart?.result?.[0];
        const closes = chart?.indicators?.quote?.[0]?.close?.filter((value: number | null): value is number => typeof value === 'number' && Number.isFinite(value)) ?? [];

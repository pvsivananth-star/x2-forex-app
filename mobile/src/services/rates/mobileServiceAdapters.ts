...
export type LegacyFetchedMap = Record<
    string,
    LegacyFetchedQuote
>;
...
export async function fetchFxForMobileService(
    tenor: Tenor,
): Promise<LegacyFetchedMap> {
    return toLegacyMap(
        await fetchFxRates(tenor),
    );
}
...
export async function fetchMetalsForMobileService(
    tenor: Tenor,
): Promise<LegacyFetchedMap> {
    return toLegacyMap(
        await fetchMetalsRates(tenor),
    );
}

export async function fetchEquityForMobileService(): Promise<LegacyFetchedMap> {
    return toLegacyMap(
        await fetchEquityRates(),
    );
}

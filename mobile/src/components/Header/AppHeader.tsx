...
export function AppHeader({title, onMenuPress}: {title: string; onMenuPress?: () => void}) {
    ...
    const refresh = useMobileStore(s => s.refresh);
    return (
        <View style={styles.root}>
            <Pressable onPress={onMenuPress} accessibilityRole="button" accessibilityLabel="Open menu">
                <Text style={styles.menu}>☰</Text>
            </Pressable>
            <Text style={styles.title}>{title}</Text>
            <View style={styles.actions}>
                <ConnectionIndicator online={online}/>
                <RefreshButton onPress={() => void refresh()}/>
            </View>
        </View>
    );
}

                ...
                <View style={styles.headerRow}>
                    ...
                    <View style={styles.headerActions}>
                        {activeTab !== 'portfolio' && activeTab !== 'equity' && !isEditMode && (
                            <TouchableOpacity onPress={startEditing} style={styles.headerButton} accessibilityLabel="Edit watchlist">
                                <Text style={[styles.headerButtonText, {color: colors.text}]}>✎</Text>
                            </TouchableOpacity>
                        )}
                        {activeTab !== 'portfolio' && !isEditMode && (
                            <RefreshTimer countdown={countdown} totalSeconds={REFRESH_INTERVAL_SECONDS} color={colors.accent} backgroundColor={colors.border} disabled={isLoading} onPress={() => void forceRefresh()}/>
                        )}
            ...
            {activeTab === 'portfolio' ? (
                ...
                <>
                    ...
                    {tenorOpen && !isEditMode && (
                        <View style={[styles.tenorMenu, {backgroundColor: colors.surfaceElevated, borderColor: colors.border}]}>
                            {TENOR_OPTIONS.map(option => (
                                <TouchableOpacity key={option} onPress={() => changeTenor(option)} style={[styles.tenorOption, option === activeTenor && {backgroundColor: colors.accentStrong}]}>
                                    <Text style={{color: option === activeTenor ? '#FFFFFF' : colors.text, fontWeight: '800' as const}}>{option}</Text>
                                </TouchableOpacity>
                            ))}
                        </View>
                    )}
                    <ScrollView style={styles.content} contentContainerStyle={isEditMode ? styles.editContent : styles.marketContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator persistentScrollbar>
                        ...
                        {visibleAssets.map((asset, index) => isEditMode ? (
                            <EditRow key={asset.symbol} asset={asset} index={index} count={visibleAssets.length} locked={(activeTab === 'fx' || activeTab === 'crypto' || activeTab === 'metals') && asset.symbol === 'USD'} colors={colors} onMove={moveRow} onRemove={removeAsset}/>
                        ) : (
                            <MarketRow key={asset.symbol} asset={asset} decimalPlaces={decimalPlaces} colors={colors} draftValue={draftRates[asset.symbol]} onDraftChange={changeDraftRate} onCommit={commitRate} active={asset.symbol === focusedSymbol} onActivate={() => setFocusedSymbol(asset.symbol)} onDeactivate={() => setFocusedSymbol(null)}/>
                        ))}
                    ...
                    {isEditMode && <View style={[styles.editFooter, {backgroundColor: colors.surface, borderTopColor: colors.border}]}>
                        <TouchableOpacity onPress={cancelEditing} style={[styles.footerButton, {borderColor: colors.border}]}><Text style={{color: colors.muted, fontWeight: '900' as const}}>Cancel</Text></TouchableOpacity>
                        <TouchableOpacity onPress={() => void applyEditing()} style={[styles.footerButton, {backgroundColor: colors.accentStrong, borderColor: colors.accentStrong}]}><Text style={{color: '#FFFFFF', fontWeight: '900' as const}}>Apply</Text></TouchableOpacity>
                    </View>}
                </>
            )}
...
const styles = StyleSheet.create({
    container: {flex: 1},
    header: {paddingHorizontal: 10, paddingVertical: 9, borderBottomWidth: 1},
    headerRow: {minHeight: 38, flexDirection: 'row', alignItems: 'center'},
    menuButton: {width: 34, height: 34, alignItems: 'center', justifyContent: 'center'},
    menu: {fontSize: 21, fontWeight: '700'},
    logo: {fontSize: 21, fontWeight: '900', letterSpacing: 1, marginHorizontal: 8},
    ...
    headerButtonText: {fontSize: 20, fontWeight: '800'},
    editingLabel: {fontSize: 10, fontWeight: '900', letterSpacing: 1, marginRight: 5},
    connectionIndicator: {width: 10, height: 10, borderRadius: 5, marginLeft: 5},
    tableHeader: {flexDirection: 'row', alignItems: 'center', minHeight: 34, paddingHorizontal: 10, borderBottomWidth: 1},
    headerCell: {fontSize: 10, fontWeight: '900', textTransform: 'uppercase'},
    assetHeader: {flex: 2},
    rateHeader: {flex: 1.55, textAlign: 'right'},
    tenorHeader: {flex: 0.95, alignItems: 'flex-end'},
    tenorMenu: {position: 'absolute', right: 10, top: 83, zIndex: 100, borderWidth: 1, borderRadius: 9, overflow: 'hidden', elevation: 8},
    tenorOption: {paddingHorizontal: 20, paddingVertical: 10},
    content: {flex: 1, paddingHorizontal: 10},
    marketContent: {paddingBottom: 10},
    editContent: {paddingBottom: 15},
    editInfo: {borderWidth: 1, borderRadius: 9, paddingHorizontal: 11, paddingVertical: 9, marginVertical: 8},
    editInfoText: {fontSize: 11, lineHeight: 16},
    addButton: {marginTop: 12, minHeight: 45, borderWidth: 1, borderRadius: 9, alignItems: 'center', justifyContent: 'center'},
    addText: {fontSize: 13, fontWeight: '900'},
    editFooter: {flexDirection: 'row', gap: 9, paddingHorizontal: 10, paddingVertical: 8, borderTopWidth: 1},
    footerButton: {flex: 1, minHeight: 42, alignItems: 'center', justifyContent: 'center', borderRadius: 8, borderWidth: 1},
    portfolio: {flex: 1, alignItems: 'center', justifyContent: 'center'},
    portfolioIcon: {fontSize: 44, marginBottom: 10},
    portfolioTitle: {fontSize: 20, fontWeight: '900'},
    portfolioText: {marginTop: 5, fontSize: 13, fontWeight: '700'},

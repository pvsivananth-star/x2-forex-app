...
export const MobileApplication: React.FC =
    () => {
        ...
        const changeDraftRate = (symbol: string, value: string,) => {
            setDraftRates((previous) => ({ ...previous, [symbol]: value }));
        };
        ...
        const moveRow = (index: number, direction: -1 | 1,) => {
            ...
            if ((activeTab === 'fx' || activeTab === 'crypto' || activeTab === 'metals') && (next[index] === 'USD' || next[target] === 'USD')) {
                return;
            }
        ...
        const changeTenor = (value: typeof TENOR_OPTIONS[number],) => {
            setTenor(value);
            setTenorOpen(false);
        };
        ...
        return (
            <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
                ...
                <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
                    <View style={styles.headerRow}>
                        ...
                        <View style={styles.headerActions}>
                            ...
                            {isEditMode && (
                                <Text style={[styles.editingLabel, { color: colors.warning }]}>EDIT</Text>
                            )}

                            <View accessible={true} accessibilityLabel={isOnline ? 'Live' : 'Offline'} accessibilityHint={isOnline ? 'Market data is connected' : 'Market data is offline'} accessibilityRole="image" style={[styles.connectionIndicator, { backgroundColor: isOnline ? colors.positive : colors.negative }]} />
                        </View>
                    </View>
                </View>

                {/* CONTENT: delegated to screens */}
                {activeTab === 'portfolio' ? (
                    <PortfolioScreen colors={colors} />
                ) : (
                    <MarketScreen
                        activeTab={activeTab}
                ...
                {/* BOTTOM TABS */}
                {!isEditMode && (
                    <BottomTabs activeTab={activeTab} colors={colors} onChange={setActiveTab} />
                )}

                <AssetPickerModal
                    visible={pickerOpen}
                    title={`Add ${activeTab === 'fx' ? 'Currency' : activeTab === 'crypto' ? 'Crypto' : 'Metal'}`}
                    placeholder={activeTab === 'crypto' ? 'Search Bitcoin, BTC, Ethereum...' : activeTab === 'fx' ? 'Search INR, India, Euro...' : 'Search Gold, Silver, Platinum...'}
                    assets={filteredPickerCatalog}
                ...
                <SettingsModal
                    ...
                    onDecimalChange={(value) => setDecimalPlaces(value)}
